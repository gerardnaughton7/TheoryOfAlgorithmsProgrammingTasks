...
; My first function i based on checking the value at each element rather than adding of values. I did this
; function this way to try out or and and statements which i have not yet used
; sod2 function
...
; My second function uses adding and modulo function to work out if the elements in the list contains a odd or even number of 1s
; sod22 function
(define (sod22 x y z); Declare function and take in 3 lists
  (if(null? x);check if x list is equal to null(if first time return empty list. otherwise return list with correct element in proper positions)
  '()
  (if(= 0 (modulo (+(car x) (car y) (car z))2));check if the sum of first element in the 3 list modulo 2 = 0
     (cons 1 (sod22 (cdr x) (cdr y) (cdr z)));if any of the statements are true above cons 1 to the function call sod2 using the cdr(lists minus the first element) of the 3 lists
     (cons 0 (sod22 (cdr x) (cdr y) (cdr z)));if any of the statements are false above cons 0 to the function call sod2 using the cdr(lists minus the first element) of the 3 lists
  )
  )
)  
...
(displayln '(sod22 results on (x y z) and (a b c)))
(sod22 x y z)
(sod22 a b c)
